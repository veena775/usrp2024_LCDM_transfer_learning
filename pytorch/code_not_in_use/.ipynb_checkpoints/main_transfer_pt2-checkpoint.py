...
class Objective(object):
    def __init__(self, input_size, output_size, max_layers, max_neurons_layers, device, 
                 epochs, seed, batch_size, max_epochs_no_improvement=50):
        ...
        self.max_no_change = 5  # Number of trials before stopping if no change
        
        self.mother = '/scratch/network/vk9342/USRP2024/pytorch/Pk_nwLH/dynamic_transfer/'
        # self.mother = '/scratch/network/vk9342/USRP2024/pytorch/Pk_LH/dynamic/'
    ...
    def __call__(self, trial):
        ...
        model = architecture.dynamic_model(trial, self.input_size, self.output_size,
                                           self.max_layers,
                                           self.max_neurons_layers).to(self.device)
        ...
        train_loader = data.create_dataset('train', self.seed, f_Pk, f_Pk_norm, 
                                           f_params, self.batch_size, shuffle=True, workers=1, cosm_type = cosm_type)
        valid_loader = data.create_dataset('valid', self.seed, f_Pk, f_Pk_norm, 
                                           f_params, self.batch_size, shuffle=False, workers=1, cosm_type = cosm_type)
...
f_Pk      = 'results/final_hidden_layer/Results_'+mode+'_Pk_nwLH_params_dynamic_first_5.txt' #'all_Pk_nwLH.npy'
f_params  = '../real_params/latin_hypercube_params_nwLH.txt' 
study_name       = 'Pk_nwLH_params_dynamic_transfer'   #+str(epochs)
output_size        = 7
final_hidden_units = 5 
...
objective = Objective(input_size, output_size, max_layers, max_neurons_layers, 
                      device, epochs, seed, batch_size, final_hidden_units)
