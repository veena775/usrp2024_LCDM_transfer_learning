...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        # get the data
        full_train_dataset_BSQ = data.create_dataset('train', self.seed, f_Pk_BSQ, f_Pk_norm, 
                                           f_params_BSQ, self.batch_size, shuffle=False, 
        ...
        train_subset_BSQ, _ = torch.utils.data.random_split(full_train_dataset_BSQ,
                                                        [n_sims_BSQ, len(full_train_dataset_BSQ) - n_sims_BSQ],
                                                        generator=torch.Generator().manual_seed(self.seed)
                                                       )

        full_train_dataset_cosm = data.create_dataset('train', self.seed, f_Pk_cosm, f_Pk_norm, 
                                           f_params_cosm, self.batch_size, shuffle=False, 
                                           workers=1, cosm_type = cosm_type_data_range, log=log, 
                                           shuffle_all=True).dataset
        
        train_subset_cosm, _ = torch.utils.data.random_split(full_train_dataset_cosm,
                                                        [self.n_sims, len(full_train_dataset_cosm) - self.n_sims],
                                                        generator=torch.Generator().manual_seed(self.seed)
                                                       )
        combined_train_dataset = torch.utils.data.ConcatDataset([train_subset_BSQ, train_subset_cosm])
        
        
        w_lcdm = 1.0 / n_sims_BSQ
        w_beyond = 1.0 / self.n_sims
        weights = torch.tensor([w_lcdm]*n_sims_BSQ + [w_beyond]*self.n_sims)
        ...
        train_loader = DataLoader(combined_train_dataset, batch_size=self.batch_size, 
                                  sampler=sampler, num_workers=1)  #shuffle = True
        


        
        valid_loader_BSQ = data.create_dataset('valid', self.seed, f_Pk_BSQ, f_Pk_norm, 
                                           f_params_BSQ, self.batch_size, shuffle=False,
                                           workers=1, cosm_type = cosm_type_data_range, log=log, 
                                           shuffle_all = True)
        valid_loader_cosm = data.create_dataset('valid', self.seed, f_Pk_cosm, f_Pk_norm, 
                                           f_params_cosm, self.batch_size, shuffle=False,
                                           workers=1, cosm_type = cosm_type_data_range, log=log, 
                                           shuffle_all = True)
        
        combined_valid_dataset =torch.utils.data.ConcatDataset([valid_loader_BSQ.dataset, valid_loader_cosm.dataset])
        valid_loader = DataLoader(combined_valid_dataset, batch_size=self.batch_size, shuffle=False, num_workers=1)
...
if __name__ == "__main__":
    ...
    additional_extension = '_combined_weighted'
    n_sims_BSQ = 22000
    log = True
    if k_max == 0.5:
        input_size = 79
